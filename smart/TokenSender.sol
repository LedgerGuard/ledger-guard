...
contract TokenSender is ExpiryHelper {
    mapping(address => mapping(address => bool)) exhausted;
    ...
    function transferPrecompile(address tokenId, address receiver, int64 amount) external {
        if (exhausted[address(this)][receiver]) {
            revert ("Vote exhausted by prior selection");
        }
        ...
        exhausted[address(this)][receiver] = true;
    }
