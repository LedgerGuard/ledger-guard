...
function environmentSetup(account_id, private_key) {
...
async function htsContractDemo() {
    //Grab your Hedera testnet account ID and private key from your .env file
    const accountIdTest = AccountId.fromString(process.env.MY_ACCOUNT_ID);
    const accountKeyTest = PrivateKey.fromStringECDSA(process.env.MY_PRIVATE_KEY);
    const client = environmentSetup(accountIdTest, accountKeyTest)
